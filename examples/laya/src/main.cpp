...
static void print_help(const char* argv0) {
    ...
    std::cout
        ...
        << "  --device <NAME>         auto | cpu | cuda | cuda:0 | metal | vulkan | vulkan:0  (default: auto)\n"
        ...
        << "              [--max-batch <N>]\n\n"
        << "  Start the Decision Studio (GET /) and the TypeSafe API (POST /v1/systemone).\n\n"
        << "  Auth is off unless LAYA_API_KEY or TYPESAFE_API_KEY is set (Bearer token).\n"
        << "  Point typesafe-sdk at this process with api_key='local' and\n"
        ...
        << "  --device <NAME>         auto | cpu | cuda | metal | vulkan | vulkan:0  (default: auto)\n"
        << "  --threads <N>           CPU workers  (default: 4)\n"
        << "  --cuda-graph            Capture a CUDA graph for the live (B, S) shape\n"
        << "  --max-batch <N>         Cap questions per forward  (default: from GGUF)\n\n"
        << "DAEMON\n"
