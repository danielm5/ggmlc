...
def numpy(
    tensor: ffi.CData, allow_copy: bool | np.ndarray = False, allow_requantize=False
) -> np.ndarray:
    """
...
def __type_to_dtype(type: int) -> np.dtype | None:
...
def __get_type(tensor: TensorLike):
    return __dtype_to_type(tensor.dtype) if isinstance(tensor, np.ndarray) else tensor.type


def __get_shape(x: TensorLike):
    return x.shape if isinstance(x, np.ndarray) else tuple([x.ne[i] for i in range(x.n_dims)])


def __get_strides(x: TensorLike):
    return x.strides if isinstance(x, np.ndarray) else tuple([x.nb[i] for i in range(x.n_dims)])
...
def __expect_same_layout(name1: str, tensor1: TensorLike, name2: str, tensor2: TensorLike):
    ...
    assert __is_contiguous(tensor1) and __is_contiguous(tensor2), (
...
def __check_shape_consistent_with_type(tensor: TensorLike):
    ...
    assert not (block_size == 0 and type in __k_quant_types), (
        "Can't quantize, native library was not compiled with USE_K_QUANTS!"
    )
